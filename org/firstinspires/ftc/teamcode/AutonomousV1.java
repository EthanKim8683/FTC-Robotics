...
public class AutonomousV1 extends LinearOpMode {
  ...
  private double armUpperBound = 0.7;
  private double clawLowerBound = 0.5;
  private double clawUpperBound = 0.85;
  private double frontArmLowerBound = 0.6;
  ...
  // Position Parameters
  private double[] armPositions = { 0.0, 1.0 };
  ...
  private void initProcesses() {
    ...
    // Create async events
    this.startEntry = new Async();
    Async moveToPosition1 = new Async(async -> {
      this.odometryWrapper.subscribePoseEvent((x, y, r) -> {
        this.mecanumWrapper.setPowerY(0.5);
        if (y < 60.0) return false;
        this.mecanumWrapper.setPowerY(0.0);
        async.finish();
        return true;
      });
    });
